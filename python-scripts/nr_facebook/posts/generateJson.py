...
def generate_json(theme, number_posts, min_comments, max_comments,file, precisions=""):
    ...
    prompt = f'Écris {str(number_posts)} descriptions pour {str(number_posts)} posts facebook sur le thème: {theme} sous forme de json. Tu peux différer légérement du thème en parlant de quelque chose en rapport. Chaque posts a un nombre de commentaires aléatoire entre {str(min_comments)} et {str(max_comments)}. Change de nombre de commentaires pour chaques post. Certains commentaires ont des réponses, d autres non. Crée une ou deux réponses pour certains commentaires.'
    ...
    format_instructions =  'Suis ce format de json: {"posts":[{"description":"...","comments":[{"content":"...", "responses":[]}, {"content":"...", "responses":[{"response":"..."}]}]},{"description":"...","comments":[{"content":"...", "responses":[{"response":"..."}]}, {"content": "...", "responses":[]}]} mais fais absolument en sorte qu il y ait un nombre de commentaires et de réponses qui change en fonction des posts'
    warnings = 'Ne réponds que par le json sous forme de string, pas de markdown pour indiqué que c est un json ni de phrases. Ne te répète pas et sois original dans les descriptions, les commentaires et les réponses. Tu peux être négatif, comme sur un vrai réseau social. Ne mentionne pas forcément le thème in texto. Le plus important est d être réaliste.'
...
max_comments = 5
precisions = ""
file = 'python-scripts/nr_facebook/posts/posts.json'
